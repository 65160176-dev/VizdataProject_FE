<template>
  <div class="col-sm-12">
    <div class="card shadow-sm border-0 overflow-hidden">
      <div
        class="card-header d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center py-3 bg-gradient-orange text-white gap-3"
      >
        <h5
        ...
        <div
          class="d-flex flex-wrap gap-2 w-100 w-md-auto justify-content-start justify-content-md-end"
        >
          <button
            type="button"
            class="btn btn-success flex-fill flex-md-grow-0 shadow-sm border border-white"
          ...
          <button
            type="button"
            class="btn btn-info text-white flex-fill flex-md-grow-0 shadow-sm border border-white"
            data-bs-toggle="modal"
          ...
          <button
            type="button"
            class="btn btn-light text-orange fw-bold flex-fill flex-md-grow-0 shadow-sm"
            data-bs-toggle="modal"
      ...
      <div class="card-body p-0">
        <div class="product-physical table-responsive">
          <table
            class="table table-hover text-center align-middle mb-0"
          ...
          >
            ...
            <tbody v-if="products && products.length > 0">
              ...
              >
                <td class="ps-4">
                  <img
                    class="img-40 me-2 shadow-sm"
                    :src="item.image || 'https://placehold.co/400'"
                    style="border-radius: 8px"
                  />
                </td>
                <td
                  class="fw-bold text-dark text-start"
                ...
                <td>
                  <span
                    class="badge bg-light text-dark border fw-normal px-2 py-1"
                    >{{ item.stock }}</span
                  >
                </td>
                ...
                <td>
                  <span
                    class="badge bg-success-subtle text-success border fw-normal px-2 py-1"
                ...
                <td>
                  <span
                    class="badge bg-orange-subtle text-orange border-0 px-3 py-2"
                ...
                <td class="pe-4">
                  <button
                    class="btn btn-sm btn-light text-secondary me-1"
                    @click="goEdit(item._id)"
                    title="Edit"
                  >
                    <Icon name="feather:edit-2" size="16" />
                  </button>
                  <button
        ...
        <div class="modal fade" id="categoryModal" tabindex="-1">
          <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg bg-light">
              <div
                class="modal-header border-0 pb-3 pt-4 px-4 d-flex flex-column align-items-start bg-white rounded-top-3 shadow-sm position-relative"
                style="z-index: 10"
              >
                <button
                  class="btn-close d-none d-md-block position-absolute"
                  style="top: 24px; right: 24px; z-index: 20"
                ...
                <div
                  class="d-flex flex-column flex-sm-row align-items-stretch align-items-sm-center gap-2 w-100 mt-md-0 position-md-absolute end-md-0 top-md-4 pe-md-5 me-md-4"
                  style="max-width: 100%; width: auto"
                >
                  <div
                    class="input-group input-group-sm rounded-pill border overflow-hidden bg-light category-search-box"
                  >
                    <span
                      class="input-group-text bg-transparent border-0 text-muted ps-3"
                      ><Icon name="feather:search" size="14"
                    /></span>
                    <input
                      type="text"
                  ...
                  <button
                    class="btn btn-sm btn-primary rounded-pill px-3 d-flex justify-content-center align-items-center gap-1"
              ...
              <div class="modal-body p-0">
                <div
                  ...
                  class="px-4 py-3 bg-white border-bottom collapse-transition shadow-sm"
                  style="z-index: 5; position: relative"
                >
                  <label class="form-label small fw-bold text-dark"
                    >ชื่อ Category ที่ต้องการเพิ่ม:</label
                  >
                  <div class="input-group">
                    <input
                      type="text"
                      class="form-control rounded-start-pill ps-3 border-orange-focus shadow-none"
                      placeholder="พิมพ์ชื่อหมวดหมู่... (สูงสุด 30 ตัวอักษร)"
                      v-model="newCategoryInput"
                      @keyup.enter="addNewCategoryToSystem"
                      maxlength="30"
                    />
                    <button
                      class="btn btn-primary rounded-end-pill px-4"
                      @click="addNewCategoryToSystem"
                    >
                      บันทึก
                    </button>
                  </div>
                ...
                >
                  <h6 class="fw-bold text-secondary mb-3">
                    <Icon name="feather:grid" size="16" class="me-1 mb-1" />
                    หมวดหมู่ระบบ
                  </h6>

                  <div
                    class="category-scroll-area mb-4"
                    style="
                      max-height: 340px;
                      overflow-y: auto;
                      overflow-x: hidden;
                  ...
                  >
                    <div class="row g-3">
                      <div
                        v-for="cat in filteredSystemCategories"
                        :key="cat.id"
                        class="col-12 col-sm-6 col-md-4"
                      >
                        <div
                          class="form-check p-3 bg-white rounded-3 shadow-sm d-flex justify-content-between align-items-center h-100 transition-all m-0"
                        ...
                        >
                          ...
                          >
                            <input
                              class="form-check-input me-2 ms-0 mt-0"
                              type="checkbox"
                              :checked="cat.isSelected"
                            />
                            <label
                              class="form-check-label fw-bold mb-0 text-dark"
                      ...
                      <div
                        v-if="filteredSystemCategories.length === 0"
                        class="col-12 text-center text-muted small py-4 bg-white rounded-3 border border-dashed"
                      >
                  ...
                  <div
                    class="category-scroll-area pb-2"
                    style="
                      max-height: 250px;
                      overflow-y: auto;
                      overflow-x: hidden;
                  ...
                  >
                    <div class="row g-3">
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <button
                              type="button"
                              class="btn btn-sm btn-link text-danger p-1 m-0"
                              @click.stop.prevent="openConfirmDelete(cat)"
                              title="ลบถาวร"
                            >
                              <Icon
                      ...
                      <div
                        v-if="filteredCustomCategories.length === 0"
                        class="col-12 text-center text-muted small py-4 bg-white rounded-3 border border-dashed"
                      >
                        {{
                          categorySearch
                            ? "ไม่พบหมวดหมู่ที่ค้นหา"
                            : "ยังไม่มีหมวดหมู่ที่เพิ่มเข้ามาเอง"
                        }}
                      </div>
                    </div>
                  </div>
            ...
            <div class="modal-content border-0 shadow-lg text-center">
              <div class="modal-body pt-5 pb-4">
                ...
                <p class="text-muted small">
                  หมวดหมู่ "{{ categoryToDelete?.name }}" จะหายไปจากระบบถาวร
                </p>
            ...
            <div class="modal-content border-0 shadow">
              ...
              <div class="modal-body py-4">
                <form>
                  <div class="row">
                    <div
                      class="col-md-5 d-flex flex-column align-items-center justify-content-center mb-3"
                    >
                      <div class="upload-box" @click="triggerFileInput('add')">
                        <img
                          v-if="newItem.previewImage"
                      ...
                      <div class="row">
                        <div class="col-12 col-sm-6 form-group mb-3">
                          ...
                          <input
                            ...
                            @input="
                              newItem.stock < 0 ? (newItem.stock = 0) : null
                            "
                          />
                        </div>
                        <div class="col-12 col-sm-6 form-group mb-3">
                                ...
                                :class="[
                                  newItem.shippingCost === option.value
                                    ? 'btn-' +
                                      option.colorName +
                                      ' active-badge'
                                    : 'btn-outline-' + option.colorName,
                                ]"
                                @click="newItem.shippingCost = option.value"
                              >
                                {{ option.text }}
                                <Icon
                                  v-if="newItem.shippingCost === option.value"
                                  name="feather:check"
                                  size="12"
                                />
                              </button>
                  ...
                  <div class="row">
                    ...
                    >
                      <div class="upload-box" @click="triggerFileInput('edit')">
                        <img
                          v-if="editItem.previewImage"
                          :src="editItem.previewImage"
                          class="img-preview"
                        />
                        <img
                          v-else-if="editItem.image"
                          :src="editItem.image"
                          class="img-preview"
                        />
                        <div v-else class="text-center text-muted">
                      ...
                      <div class="row">
                        <div class="col-12 col-sm-6 form-group mb-3">
                          ...
                          <input
                            ...
                            @keydown="preventNegativeInput"
                            @input="
                              editItem.stock < 0 ? (editItem.stock = 0) : null
                            "
                          />
                        </div>
                        <div class="col-12 col-sm-6 form-group mb-3">
                          ...
                          <div class="mt-2" v-if="editItem.weight >= 0">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                              ...
                              >
                                {{ option.text }}
                                <Icon
                                  v-if="editItem.shippingCost === option.value"
                                  name="feather:check"
                                  size="12"
                                />
                              </button>
            ...
            <div class="modal-content border-0 shadow">
                ...
                <form>
                  ...
                  <div class="form-group mb-3">
                    ...
                    >
                      ...
                      <option
                        v-for="item in filteredStockProducts"
                        :key="item._id"
                        :value="item._id"
                      >
                        {{ item.name }}
                    ...
                    <div
                      ...
                      class="d-flex align-items-center p-2 border rounded-3 bg-white shadow-sm mt-2 transition-all"
                    >
                      <img
                        :src="
                          selectedStockProduct.image ||
              ...
              <div class="modal-footer border-0">
                <button
                  ...
                  @click="saveAddStock"
                  :disabled="!stockForm.id || !stockForm.quantity"
                >
                  Update Stock
                </button>
                <button
...
const fetchCategories = async () => {
  try {
    ...
    const res = await $fetch(
      "https://vizdataprojectbe-production.up.railway.app/api/category",
      {
...
const toggleCategory = async (cat) => {
  ...
  if (cat.isSelected) {
    if (checkIsCategoryUsed(cat.name)) {
      showError(
        `ไม่สามารถซ่อนหมวดหมู่ "${cat.name}" ได้ เนื่องจากยังมีสินค้าในหมวดหมู่นี้`,
      );
      return;
    }
    ...
    try {
      await $fetch(
        `https://vizdataprojectbe-production.up.railway.app/api/category/${cat.id}`,
        { method: "DELETE", headers: { Authorization: `Bearer ${token}` } },
      );
    } catch (e) {
      cat.isSelected = true;
      showError("เกิดข้อผิดพลาดในการยกเลิกหมวดหมู่");
    }
  } else {
    cat.isSelected = true;
    try {
...
const openConfirmDelete = (catObj) => {
  ...
  if (checkIsCategoryUsed(catObj.name)) {
    showError(
      `ไม่สามารถลบหมวดหมู่ "${catObj.name}" ได้ เนื่องจากยังมีสินค้าอยู่`,
    );
    return;
  }
...
const executeDeleteCategory = async () => {
  ...
  if (!item) return;
  const modalInstance = bootstrap
    ? bootstrap.Modal.getInstance(document.getElementById("confirmDeleteModal"))
    : null;
  const token = localStorage.getItem("token");

  try {
    // ถ้าหมวดหมู่ถูกติ๊กอยู่ ให้ยิง API ลบไปตามปกติ
    if (item.isSelected) {
      await $fetch(
        `https://vizdataprojectbe-production.up.railway.app/api/category/${item.id}`,
        { method: "DELETE", headers: { Authorization: `Bearer ${token}` } },
      );
    }

    // ลบออกจากอาเรย์หน้าจอทันที
    allCategories.value = allCategories.value.filter((c) => c.id !== item.id);
    showToast("ลบหมวดหมู่ถาวรเรียบร้อย");
  } catch (e) {
    console.error(e);
    // ดัก Error: ถ้าหากว่ามัน "ไม่ได้ติ๊กอยู่" (แปลว่า toggleCategory สั่ง DELETE ไปแล้วครั้งนึง)
    // ถ้ายิงซ้ำแล้วมัน Error (หาไม่เจอ) ก็ปล่อยผ่านไปเลย และเคลียร์ออกจาก UI ให้
    if (!item.isSelected) {
      allCategories.value = allCategories.value.filter((c) => c.id !== item.id);
      showToast("ลบหมวดหมู่ถาวรเรียบร้อย");
    } else {
...
const fetchProducts = async () => {
  try {
    ...
    if (!token) return;
    const res = await $fetch(
      "https://vizdataprojectbe-production.up.railway.app/api/product/my-products",
      { headers: { Authorization: `Bearer ${token}` } },
    );
    products.value = res || [];
  } catch (e) {
...
const getStockStatus = (s) =>
  s >= 100 ? "success" : s >= 25 ? "warning" : "danger";
...
const calculateOptions = (w) => {
  ...
  if (weight >= 1)
    options.push({ value: 30, text: "30 บ.", colorName: "ship-30" });
  if (weight >= 10)
    options.push({ value: 60, text: "60 บ.", colorName: "ship-60" });
  if (weight >= 20)
    options.push({ value: 120, text: "120 บ.", colorName: "ship-120" });
  return options;
};

const triggerFileInput = (m) =>
  document.getElementById(`${m}FileInput`).click();
const onFileChange = (e, m) => {
  const f = e.target.files[0];
  if (f) {
    const p = URL.createObjectURL(f);
    if (m === "add") {
      newItem.value.previewImage = p;
      newItem.value.rawFile = f;
    } else {
      editItem.value.previewImage = p;
      editItem.value.rawFile = f;
    }
  }
};
...
watch(
  ...
  (v) => {
    ...
    if (availableOptionsAdd.value.length > 0)
      newItem.value.shippingCost =
        availableOptionsAdd.value[availableOptionsAdd.value.length - 1].value;
  },
);

watch(
  () => editItem.value.weight,
  (v) => {
...
const saveNewItem = async () => {
  ...
  try {
    ...
    console.error(e);
    showError("เพิ่มสินค้าไม่สำเร็จ");
  }
};
...
async function saveEdit() {
  ...
  try {
    await $fetch(
      `https://vizdataprojectbe-production.up.railway.app/api/product/${editItem.value._id}`,
      {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
        body: fd,
...
async function deleteItem(id) {
  if (confirm("ลบสินค้า?")) {
    const token = localStorage.getItem("token");
    await $fetch(
      `https://vizdataprojectbe-production.up.railway.app/api/product/${id}`,
      { method: "DELETE", headers: { Authorization: `Bearer ${token}` } },
    );
...
async function saveAddStock() {
  ...
  if (t) {
    ...
    try {
      await $fetch(
        `https://vizdataprojectbe-production.up.railway.app/api/product/${t._id}`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
          body: fd,
...
:deep(.text-orange-subtle) {
  color: #fcc495;
}
:deep(.bg-orange-subtle) {
  background-color: #fff4e6 !important;
}
:deep(.border-orange) {
  border-color: #fd7e14 !important;
}
:deep(.border-orange-focus:focus) {
  border-color: #fd7e14 !important;
...
.category-scroll-area::-webkit-scrollbar-track {
  background: transparent;
}
.category-scroll-area::-webkit-scrollbar-thumb {
  background: #cbd5e1;
  border-radius: 10px;
}
.category-scroll-area::-webkit-scrollbar-thumb:hover {
  background: #94a3b8;
}
